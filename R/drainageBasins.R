...
#' @param streams Optionally, the path to the polylines shp or gpkg file containing streams, which can be used to improve accuracy when using poor quality DEMs. If this shapefile is the only input parameter being modified from previous runs (i.e. you've found a new/better streams shapefile but the DEM is unchanged) then specify a file here and overwrite = TRUE.
...
#' @param points The path to the points shapefile or geopackage file (extension .shp or .gpkg) containing the points from which to build watersheds. The attribute of each point will be attached to the newly-created drainage polygons. Leave NULL (along with related parameters) to only process the DEM without defining watersheds. Note that shapefile attribute column names are limited to 10 characters.
...
#'
#' @return A list of {terra} objects and saved files. If points are specified: delineated drainages, pour points as provided, snapped pour points, and the derived streams network. If no points: flow accumulation and direction rasters, and the derived streams network. If points specified, also saved to disk: a geopackage file for each drainage basin, plus the associated snapped pour point and the point as provided and a geopackage file for all basins/points together. In all cases the created rasters will be in the same folder as the DEM and in the geoTIFF format.
#'
...
drainageBasins <- function(DEM, streams = NULL, breach_dist = 10000, threshold = 500, overwrite = FALSE, points = NULL, points_name_col = NULL, projection = NULL, snap = "nearest", snap_dist = 200, save_path = "choose", force_update_wbt = FALSE) {
  ...
  if (!dir.exists(save_path)){
    stop("The save path (folder) you specified does not seem to exist. Do you need to create the folder, or did you point to a file?")
  }
  if (!file.exists(DEM)){
  ...
  if (wbt_check){
    ...
    message("Using WhiteboxTools version ", substr(version[1], 16, 20), ". If this is out of date, run function with force_update_wbt = TRUE.")
  } else {
  ...
  if (force_update_wbt) {
    ...
    message("Installed WhiteboxTools version ", substr(version[1], 16, 20), " (force update).")
  }
  ...
  if (!streams_derived_exists | !d8pntr_exists | !d8fac_exists | overwrite){
    ...
    message("Filling single-cell pits in the DEM...")
    ...
    if (!is.null(streams)){ #load streams, process to raster, and burn-in the DEM
      message("Creating a stream raster from the provided stream shapefile...")
